...
st.write("Upload your Plant's Image and get predictions if it is healthy or not")
...
if uploaded_file != None:
    ...
    progress = st.text("Scanning Image")
    ...
    st.image(np.array(Image.fromarray(
        np.array(image)).resize((700, 400), Image.BILINEAR)), width=None)
    ...
    # Show the results
    st.title(f"The plant {result['status']} with {result['prediction']} prediction.")
